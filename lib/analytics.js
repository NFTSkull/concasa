...
/**
 * Trackea click específico en el botón hero CTA de WhatsApp
 * 
 * @param {Object} params - Parámetros del evento
 * @param {string} params.linkUrl - URL del enlace (href del botón)
 */
export function trackHeroCTAButtonClick({ linkUrl }) {
  // Solo ejecutar en cliente (navegador)
  if (typeof window === 'undefined' || typeof gtag === 'undefined') {
    if (window && window.location && window.location.hostname === 'localhost') {
      console.warn('[analytics] gtag no disponible para tracking del botón hero CTA');
    }
    return;
  }
  ...
  // Disparar evento de GA4
  gtag('event', 'whatsapp_click', {
    button_name: 'calcular_monto_whatsapp',
    link_url: linkUrl,
    origin: 'hero',
  });
...
/**
 * Dispara evento de conversión close_convert_lead para GA4
 * 
 * @param {Object} params - Parámetros del evento
 * @param {string} params.linkUrl - URL del enlace (href del botón)
 */
export function trackCloseConvertLead({ linkUrl }) {
  try {
    // Solo ejecutar en cliente (navegador) y verificar que gtag existe
    if (typeof window === 'undefined' || typeof gtag === 'undefined') {
      return;
    }

    // Disparar evento de GA4 con transport_type: 'beacon'
    gtag('event', 'close_convert_lead', {
      link_url: linkUrl,
      link_text: 'Calcular mi monto por WhatsApp',
      placement: 'hero',
    }, {
    ...
    if (window && window.location && window.location.hostname === 'localhost') {
      console.warn('[analytics] Error al disparar close_convert_lead:', error);
    }
  }
}
